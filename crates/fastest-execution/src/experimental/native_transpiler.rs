...
impl Clone for SIMDPatternStats {
    fn clone(&self) -> Self {
        Self {
            total_patterns_analyzed: AtomicU64::new(
                self.total_patterns_analyzed.load(Ordering::Relaxed),
            ),
            ...
            average_analysis_time_ns: AtomicU64::new(
                self.average_analysis_time_ns.load(Ordering::Relaxed),
            ),
        }
    }
}
...
impl SIMDPatternAnalyzer {
    ...
    pub fn analyze_pattern_simd(&self, test_code: &str) -> TestPattern {
        ...
        self.stats
            .average_analysis_time_ns
            .store(analysis_time.as_nanos() as u64, Ordering::Relaxed);
    ...
    pub fn get_stats(&self) -> SIMDPatternStats {
        SIMDPatternStats {
            total_patterns_analyzed: AtomicU64::new(
                self.stats.total_patterns_analyzed.load(Ordering::Relaxed),
            ),
            simd_fast_path_hits: AtomicU64::new(
                self.stats.simd_fast_path_hits.load(Ordering::Relaxed),
            ),
            ast_slow_path_hits: AtomicU64::new(
                self.stats.ast_slow_path_hits.load(Ordering::Relaxed),
            ),
            cache_hits: AtomicU64::new(self.stats.cache_hits.load(Ordering::Relaxed)),
            average_analysis_time_ns: AtomicU64::new(
                self.stats.average_analysis_time_ns.load(Ordering::Relaxed),
            ),
        }
    }
...
impl NativeTestExecutor {
    ...
    pub fn execute_native_or_fallback(
        &mut self,
        test: &TestItem,
        test_code: &str,
        ...
        let result = match execution_strategy {
            ...
            ExecutionType::NativeOptimized => {
                match self.execute_template_compiled(test, test_code, pattern) {
                    ...
                    Err(_) => self.execute_pyo3_optimized(test, test_code)?,
                }
            }
    ...
    fn select_execution_strategy_simd(&self, pattern: TestPattern) -> ExecutionType {
        match pattern {
            ...
            TestPattern::ParametrizedAssertion | TestPattern::FixtureAssertion => {
                ExecutionType::PyO3Optimized
            } // Optimized PyO3
    ...
    fn execute_cached_native_fast(&self, cached_test: &CompiledTest) -> Result<NativeTestResult> {
        ...
        Ok(NativeTestResult {
            ...
            error: if passed {
                ...
                Some("Cached native assertion failed".to_string())
            },
            output: if passed {
                "PASSED (SIMD-CACHED)"
            } else {
    ...
    fn execute_template_compiled(
        ...
        Ok(NativeTestResult {
            ...
            error: if passed {
                ...
                Some("Template assertion failed".to_string())
            },
            output: if passed {
                "PASSED (TEMPLATE)"
            } else {
    ...
    fn compile_and_execute_native_jit(
        &mut self,
        test: &TestItem,
        _test_code: &str,
        ...
        Ok(NativeTestResult {
            ...
            error: if passed {
                ...
                Some("Legacy JIT assertion failed".to_string())
            },
            output: if passed {
                "PASSED (LEGACY-JIT)"
            } else {
    ...
    fn compile_and_execute_legacy_jit(
        &mut self,
        test: &TestItem,
        test_code: &str,
        ...
        // Build the function with Cranelift
        {
            ...
            let result_value = match analysis.pattern {
                TestPattern::SimpleAssertion(b) => Self::compile_simple_assertion(&mut builder, b)?,
                TestPattern::ArithmeticAssertion => {
                ...
                _ => {
                    return Err(anyhow!(
                        "Unsupported pattern for JIT compilation in match: {:?}",
                        analysis.pattern
                    )
                    .into());
                }
        ...
        let func_id = self
            .jit_module
            .declare_function(&func_name, Linkage::Local, &self.ctx.func.signature)
        ...
        let native_function: unsafe extern "C" fn() -> i32 =
            unsafe { std::mem::transmute(native_function) };
        ...
        Ok(NativeTestResult {
            ...
            error: if passed {
                ...
                Some("Native JIT assertion failed".to_string())
            },
            output: if passed {
                "PASSED (NATIVE-JIT)"
            } else {
    ...
    fn compile_arithmetic_assertion(
        ...
        if ast_analysis.pattern == TestPattern::ArithmeticAssertion {
            ...
            Err(anyhow!(
                "compile_arithmetic_assertion called with non-ArithmeticAssertion pattern: {:?}",
                ast_analysis.pattern
            )
            .into())
        }
    }
    ...
    fn compile_comparison_assertion(
        builder: &mut FunctionBuilder,
        ast_analysis: &ASTAnalysis,
    ) -> Result<Value> {
        if ast_analysis.pattern == TestPattern::ComparisonAssertion {
            ...
            Err(anyhow!(
                "compile_comparison_assertion called with non-ComparisonAssertion pattern: {:?}",
                ast_analysis.pattern
            )
            .into())
        }
    ...
    fn execute_cached_native(&self, cached_test: &CompiledTest) -> Result<NativeTestResult> {
        ...
        Ok(NativeTestResult {
            ...
            output: if passed {
                "PASSED (CACHED-NATIVE)"
            } else {
                "FAILED (CACHED-NATIVE)"
            }
            .to_string(),
            execution_type: ExecutionType::NativeJIT,
            compilation_time: None, // Already compiled
    ...
    fn compile_and_execute_native_optimized(
        &mut self,
        test: &TestItem,
        test_code: &str,
        analysis: &ASTAnalysis,
        ...
        Ok(NativeTestResult {
            ...
            error: if passed {
                ...
                Some("Native optimized assertion failed".to_string())
            },
            output: if passed {
                "PASSED (NATIVE-OPT)"
            } else {
...
impl ASTAnalyzer {
    ...
    fn analyze_test_code(&mut self, test_id: &str, test_code: &str) -> Result<ASTAnalysis> {
        ...
        let python_ast = match py_ast::Suite::parse(test_code, "<embedded_test>") {
            ...
            Err(parse_err) => {
                eprintln!(
                    "[ASTAnalyzer] Failed to parse test code for '{}': {:?}. Code:\n{}",
        ...
        self.analysis_stats.average_analysis_time = Duration::from_nanos(
            ((current_avg.as_nanos() as f64 * (count - 1.0) + analysis_time.as_nanos() as f64)
                / count) as u64,
        );
    ...
    fn recognize_pattern_from_ast(
        &self,
        ast: &py_ast::Suite,
        _test_id_for_debug: &str,
    ) -> TestPattern {
        if ast.is_empty() {
    ...
    fn calculate_complexity_from_ast(ast_option: Option<&py_ast::Suite>) -> u16 {
        if ast_option.is_none() {
            return u16::MAX;
        } // Default for parse errors or no AST
...
impl From<NativeTestResult> for TestResult {
    fn from(native_result: NativeTestResult) -> Self {
        TestResult {
            ...
            outcome: if native_result.passed {
...
mod tests {
    ...
    fn test_native_jit_simple_assertion() {
        ...
        let result = executor
            .execute_native_or_fallback(&test, "assert True")
            .unwrap();
    ...
    fn test_arithmetic_compilation() {
        ...
        let result = executor
            .execute_native_or_fallback(&test, "assert 2 + 2 == 4")
            .unwrap();
        assert!(result.passed);
    ...
    fn test_pattern_recognition() {
        ...
        let analysis = analyzer.analyze_test_code("test_1", "assert True").unwrap();
        assert_eq!(analysis.pattern, TestPattern::SimpleAssertion(true));

        let analysis = analyzer
            .analyze_test_code("test_2", "assert 2 + 2 == 4")
            .unwrap();
        assert_eq!(analysis.pattern, TestPattern::ArithmeticAssertion);

        let analysis = analyzer
            .analyze_test_code("test_3", "assert x == y")
            .unwrap();
