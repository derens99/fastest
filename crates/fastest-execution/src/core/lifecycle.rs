...
impl LifecycleManager {
    ...
    pub fn record_module_setup(&mut self, module_path: &str) -> Result<()> {
        ...
        self.setup_order
            .push((LifecycleScope::Module, module_path.to_string()));
    ...
    pub fn record_class_setup(&mut self, class_path: &str) -> Result<()> {
        ...
        self.setup_order
            .push((LifecycleScope::Class, class_path.to_string()));
    ...
    pub fn get_summary(&self) -> HashMap<String, String> {
        ...
        summary.insert(
            "active_modules".to_string(),
            self.setup_modules.len().to_string(),
        );
        summary.insert(
            "active_classes".to_string(),
            self.setup_classes.len().to_string(),
        );
        summary.insert(
            "total_scopes".to_string(),
            self.active_scopes.len().to_string(),
        );
...
impl PythonLifecycleCoordinator {
    ...
    pub fn setup_module_if_needed(
        ...
        if has_setup {
            ...
            self.manager
                .record_module_setup(module_path)
...
mod tests {
    ...
    fn test_lifecycle_order() {
        ...
        manager
            .record_class_setup("test_module::TestClass")
            .unwrap();
        assert!(!manager.needs_class_setup("test_module::TestClass"));
