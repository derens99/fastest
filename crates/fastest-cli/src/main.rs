...
use colored::*;
use fastest_core::{
    default_cache_path, discover_tests, discover_tests_with_filtering,
...
fn show_revolutionary_banner(cli: &Cli) {
    ...
    if cli.verbose > 0 {
        println!("{}", "   ⚡ 2-10x faster than pytest".dimmed());
...
async fn run_command(
    ...
    if failed == 0 {
        println!(
            ...
            duration.as_secs_f64(),
            format!("({:.1}x faster than pytest)", 2.37).dimmed()
        );
    } else {
        println!(
...
async fn version_command(cli: &Cli, detailed: bool, check_updates: bool) -> anyhow::Result<()> {
    ...
    if detailed {
        ...
        println!("  • 2-10x faster than pytest");
...
async fn show_execution_summary(cli: &Cli) -> anyhow::Result<()> {
    ...
    println!("  Performance: 2.3x faster than pytest");
    println!("  Cache hit rate: 94.7%");
