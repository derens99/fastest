...
use fastest_execution::TestResult;
...
impl DevExperienceManager {
    ...
    fn create_debug_info(&self, test: &TestItem, _result: &TestResult) -> DebugInfo {
        ...
        DebugInfo {
            ...
            debug_command,
            breakpoints: vec![format!("{}:{}", test.path.display(), test.line_number.unwrap_or(0))],
        }
    }
    ...
    fn create_ide_metadata(&self, test: &TestItem, result: &TestResult) -> IdeMetadata {
        ...
        IdeMetadata {
            ...
            line_number: test.line_number.map(|n| n as u32),
            test_type,
            status_icon,
        }
    }
    ...
    async fn display_enhanced_error(&self, test: &TestItem, result: &TestResult) -> Result<()> {
        if let Some(error) = &result.error {
            ...
            println!(
                "{}: {}",
                "Line".cyan(),
                test.line_number.map(|n| n.to_string()).unwrap_or_else(|| "?".to_string()).green()
            );
