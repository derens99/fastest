...
pub fn parse_parametrize_decorator(decorator: &str) -> Option<(Vec<String>, Vec<ParamSet>)> {
    ...
    let param_names_str = content_in_paren[..first_comma_pos].trim().trim_matches('"').trim_matches('\'');
    ...
    if param_names.is_empty() { return None; }
    ...
    } else if let Some(ids_marker_pos) = values_and_ids_str.rfind(",ids=") { // no space
    ...
    if !values_list_str.starts_with('[') || !values_list_str.ends_with(']') { return None; }
...
fn split_param_list_into_item_strings(list_content_str: &str) -> Option<Vec<String>> {
    ...
    for ch in list_content_str.chars() {
        match ch {
            '\'' | '"' => {
                if in_string && ch == string_char { in_string = false; } 
                else if !in_string { in_string = true; string_char = ch; }
                current_item_str.push(ch);
            }
            '(' if !in_string => { paren_depth += 1; current_item_str.push(ch); }
            ')' if !in_string => { paren_depth -= 1; current_item_str.push(ch); }
            '[' if !in_string => { bracket_depth += 1; current_item_str.push(ch); }
            ']' if !in_string => { bracket_depth -= 1; current_item_str.push(ch); }
    ...
    if trimmed_item.starts_with("pytest.param(") && trimmed_item.ends_with(')') {
        ...
        loop {
            ...
            if let Some(pos) = temp_content.rfind(", id=") {
                ...
                let (val_str, next_temp_content) = extract_kwarg_value_and_remaining(potential_id_val_part, temp_content, pos);
            ...
            if let Some(pos) = temp_content.rfind(", marks=") {
                ...
                let (val_str, next_temp_content) = extract_kwarg_value_and_remaining(potential_marks_val_part, temp_content, pos);
        ...
        let (marks, is_xfail) = marks_str_opt.map_or((Vec::new(), false), extract_pytest_param_marks);
        ...
        if parsed_values.len() != num_params { return None; }
        Some(ParamSet { id, values: parsed_values, marks, is_xfail })

    } else { // Regular item (not pytest.param)
        let parsed_values = if num_params == 1 {
        ...
        if parsed_values.len() != num_params { return None; }
        Some(ParamSet { id: None, values: parsed_values, marks: Vec::new(), is_xfail: false })
    }
}
...
fn extract_kwarg_value_and_remaining<'a>(
    ...
    kw_marker_pos: usize
) -> (&'a str, &'a str) { // (kwarg_value, content_before_this_kwarg)
    let mut balance = 0;
    ...
    for (i, char_code) in value_part_candidate.char_indices() {
        match char_code {
            '\'' | '"' => {
                if in_str && char_code == str_char { in_str = false; }
                else if !in_str { in_str = true; str_char = char_code; }
            }
            '(' | '[' | '{' if !in_str => balance += 1,
            ')' | ']' | '}' if !in_str => balance -= 1,
            ',' if !in_str && balance == 0 => {
...
fn extract_pytest_param_marks(marks_str: &str) -> (Vec<String>, bool) {
    ...
    if marks_str.is_empty() { return (marks, is_xfail); }

    let content = marks_str.trim_start_matches('[').trim_end_matches(']').trim();
...
fn parse_comma_separated_values(cs_str: &str) -> Option<Vec<Value>> {
    ...
    for ch in cs_str.chars() {
        match ch {
            ...
            '(' if !in_string => { paren_depth += 1; current_value_str.push(ch); }
            ')' if !in_string => { paren_depth -= 1; current_value_str.push(ch); }
            '[' if !in_string => { bracket_depth += 1; current_value_str.push(ch); }
            ']' if !in_string => { bracket_depth -= 1; current_value_str.push(ch); }
            ',' if !in_string && paren_depth == 0 && bracket_depth == 0 => {
                ...
                let cleaned_part = if !in_string && part_to_parse.contains('#') {
                    ...
                    for (i, c) in part_to_parse.char_indices() {
                        match c {
                            '\'' | '"' => {
                                if temp_in_str && c == temp_str_char { temp_in_str = false; }
                                else if !temp_in_str { temp_in_str = true; temp_str_char = c; }
                            }
...
fn parse_single_value(value_str: &str) -> Option<Value> {
    ...
    if (trimmed.starts_with('"') && trimmed.ends_with('"')) || (trimmed.starts_with('\'') && trimmed.ends_with('\'')) {
        ...
        return Some(Value::String(inner.to_string().replace("\\'", "'").replace("\\\"", "\"")));
    }
    if trimmed == "True" { return Some(Value::Bool(true)); }
    if trimmed == "False" { return Some(Value::Bool(false)); }
    if trimmed == "None" { return Some(Value::Null); }

    if (trimmed.starts_with('(') && trimmed.ends_with(')')) || (trimmed.starts_with('[') && trimmed.ends_with(']')) {
        let inner_content = &trimmed[1..trimmed.len()-1];
        if inner_content.trim().is_empty() { // Handles empty tuple () or empty list []
    ...
    if let Ok(num) = trimmed.parse::<i64>() { return Some(Value::Number(serde_json::Number::from(num))); }
    if let Ok(num) = trimmed.parse::<f64>() {
        if let Some(n) = serde_json::Number::from_f64(num) { return Some(Value::Number(n)); }
...
fn extract_ids_from_list_str(ids_list_str: &str) -> Option<Vec<String>> {
    ...
    if content.trim().is_empty() { return Some(Vec::new()); }
    Some(content.split(',')
...
pub fn expand_parametrized_tests(test: &TestItem, decorators: &[String]) -> Result<Vec<TestItem>> {
    ...
    for decorator in decorators {
        if decorator.contains("parametrize") {
            if let Some((names, param_sets)) = parse_parametrize_decorator(decorator) {
                ...
                param_info_list.push((names, param_sets, None /* TODO: Revisit overall_ids if needed distinctly here */));
            }
        }
    }
    ...
    for case in test_cases.iter() { // idx here is overall index across all combined params
        ...
        let params_json_str = if case.params.len() == 1 && case.params.keys().next().map_or(false, |k| k == "coords") {
        ...
        expanded_test.decorators.push(format!("__params__={}", params_json_str));
        if case.is_xfail {
...
fn generate_test_cases_from_param_sets(
    ...
    if param_info_list.is_empty() { return vec![]; }

    let (first_param_names, first_actual_param_sets, _first_overall_ids_decorator_level) = &param_info_list[0];
    let mut accumulated_cases: Vec<TestCase> = first_actual_param_sets
        ...
        .map(|p_set| {
            ...
            TestCase { params: params_map, id_override: p_set.id.clone(), is_xfail: p_set.is_xfail }
        })
        .collect();

    for (current_param_names, current_actual_param_sets, _current_overall_ids_decorator_level) in param_info_list.iter().skip(1) {
        ...
        for existing_case in &accumulated_cases {
            for p_set_for_current_decorator in current_actual_param_sets {
                ...
                let final_id = p_set_for_current_decorator.id.clone().or_else(|| existing_case.id_override.clone());
                let combined_xfail = existing_case.is_xfail || p_set_for_current_decorator.is_xfail;
                next_acc_cases.push(TestCase { params: new_params_map, id_override: final_id, is_xfail: combined_xfail });
            }
        }
    ...
    for key in keys {
        if let Some(value) = params.get(key) {
            let formatted = match value {
                ...
                Value::Array(arr) => format!("[{}]", arr.iter().map(format_param_id_val).collect::<Vec<_>>().join(",")).replace("-", "_"),
...
mod tests {
    ...
    fn test_parse_tuple_parametrize() {
        ...
        assert_eq!(param_sets[0].values, vec![Value::Number(1.into()), Value::Number(2.into()), Value::Number(3.into())]);
    }
    ...
    fn test_parse_coords_param() {
        let decorator = r#"@pytest.mark.parametrize("coords", [((0,0), (3,4), 5.0), ((1,1), (2,2), 1.41)])"#;
        let result = parse_parametrize_decorator(decorator);
        assert!(result.is_some());
        let (names, param_sets) = result.unwrap();
        ...
        assert_eq!(param_sets[0].values[0], Value::Array(vec![
            Value::Array(vec![Value::Number(0.into()), Value::Number(0.into())]),
            Value::Array(vec![Value::Number(3.into()), Value::Number(4.into())]),
            Value::Number(serde_json::Number::from_f64(5.0).unwrap())
        ]));
    ...
    fn test_parse_pytest_param_with_id_and_xfail_mark() {
        ...
        assert_eq!(pset.values, vec![Value::Number(1.into()), Value::String("a".to_string())]);
    ...
    fn test_parse_decorator_with_overall_ids() {
        let decorator = r#"@pytest.mark.parametrize("test_input,expected", [(1,1),(2,4)], ids=["one", "two"])"#;
        let result = parse_parametrize_decorator(decorator);
        assert!(result.is_some());
        let (names, param_sets) = result.unwrap();
