...
impl SmartCoverage {
    ...
    pub async fn collect_coverage(&mut self, test_files: &[String]) -> Result<CoverageReport> {
        ...
        // Use parallel collection for speed
        let coverage_futures: Vec<_> = test_files
            .iter()
            .map(|file| self.collect_file_coverage(file))
            .collect();

        let results = futures::future::join_all(coverage_futures).await;
