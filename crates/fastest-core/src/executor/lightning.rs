...
impl LightningExecutor {
    ...
    pub fn execute(&self, tests: Vec<TestItem>) -> Result<Vec<TestResult>> {
        ...
        let results = if total_tests <= 200 {
            ...
            if self.verbose {
                eprintln!("⚡ Executing {} tests in single lightning-fast process", total_tests);
            }
            ...
            if self.verbose {
                eprintln!("⚡ Executing {} tests in parallel lightning batches", total_tests);
            }
        ...
        if self.verbose {
            ...
            eprintln!("✅ All tests completed in {:.1}ms ({:.2}ms per test)", 
    ...
    fn build_lightning_code(&self, tests: &[TestItem]) -> String {
        ...
        code.push_str(r#"import sys,json,time,os
...
"#);
    ...
    fn parse_result(&self, json: &serde_json::Value) -> Option<TestResult> {
        ...
        let passed = json["passed"].as_u64().map(|v| v == 1)
