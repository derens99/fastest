...
impl ConftestDiscovery {
    ...
    pub fn parse_conftest(&mut self, path: &Path) -> Result<ConftestFile> {
        ...
        // Cache the result
        self.conftest_cache.insert(path.to_path_buf(), conftest.clone());
    ...
    fn parse_fixture_from_match(
        ...
        for capture in match_.captures {
            match self.fixture_query.capture_names()[capture.index as usize] {
                "function_name" => {
                    fixture_name = Some(capture.node.utf8_text(content.as_bytes()).ok()?.to_string());
                        ...
                        match arg_name {
                            "scope" => {
                                if let Ok(scope_str) = value_node.utf8_text(content.as_bytes()) {
                                    let scope_str = scope_str.trim_matches(|c| c == '"' || c == '\'');
                            ...
                            "ids" => {
                                ...
                                if value_node.kind() == "list" {
                                    for item in value_node.children(&mut value_node.walk()) {
                                        if item.kind() == "string" {
                                            if let Ok(text) = item.utf8_text(content.as_bytes()) {
                                                ids.push(text.trim_matches(|c| c == '"' || c == '\'').to_string());
                                            }
                                        }
                                    }
                                }
...
"#;
        ...
        assert!(dependent.dependencies.contains(&"simple_fixture".to_string()));
        assert!(dependent.dependencies.contains(&"module_fixture".to_string()));
