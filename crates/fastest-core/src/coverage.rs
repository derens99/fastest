...
"#,
    ...
    pub fn combine_coverage(&self) -> Result<()> {
        ...
        let output = Command::new(&self.coverage_cmd)
            ...
            .map_err(|e| crate::error::Error::Execution(
                format!("Failed to combine coverage: {}", e)
            ))?;
            
        if !output.status.success() {
            return Err(crate::error::Error::Execution(
                format!("Coverage combine failed: {}", 
    ...
    pub fn generate_report(&self, format: CoverageFormat) -> Result<CoverageReport> {
        ...
        let output = Command::new(&self.coverage_cmd)
            ...
            .output()
            .map_err(|e| crate::error::Error::Execution(
                format!("Failed to generate coverage report: {}", e)
            ))?;
            
        if !output.status.success() {
            return Err(crate::error::Error::Execution(
                format!("Coverage report failed: {}", 
                    String::from_utf8_lossy(&output.stderr))
            ));
        }
        
        // Parse the JSON report
        let json_content = std::fs::read_to_string(".coverage.json")
            .map_err(|e| crate::error::Error::Io(e))?;
        ...
        match format {
            CoverageFormat::Terminal => {
                Command::new(&self.coverage_cmd)
                    ...
                    .status()
                    .map_err(|e| crate::error::Error::Execution(
                        format!("Failed to show coverage report: {}", e)
                    ))?;
            }
            CoverageFormat::Html => {
                Command::new(&self.coverage_cmd)
                    ...
                    .status()
                    .map_err(|e| crate::error::Error::Execution(
                        format!("Failed to generate HTML coverage: {}", e)
                    ))?;
            ...
            CoverageFormat::Xml => {
                Command::new(&self.coverage_cmd)
                    ...
                    .status()
                    .map_err(|e| crate::error::Error::Execution(
                        format!("Failed to generate XML coverage: {}", e)
                    ))?;
            }
            _ => {}
        }
    ...
    fn parse_coverage_json(&self, json: &serde_json::Value) -> Result<CoverageReport> {
        ...
        if let Some(file_data) = json["files"].as_object() {
            for (filename, data) in file_data {
                ...
                let num_statements = data["summary"]["num_statements"]
                    .as_u64()
                    .unwrap_or(0) as usize;
                    
                let covered = data["summary"]["covered_lines"]
                    .as_u64()
                    .unwrap_or(0) as usize;
