...
impl Config {
    ...
    pub fn load_from_pyproject() -> Result<Self> {
        ...
        if !path.exists() {
            return Err(crate::error::Error::Config("pyproject.toml not found".to_string()));
        }
        
        let contents = fs::read_to_string(path)
            .map_err(|e| crate::error::Error::Config(format!("Failed to read pyproject.toml: {}", e)))?;
        
        let toml_value: toml::Value = toml::from_str(&contents)
            .map_err(|e| crate::error::Error::Config(format!("Failed to parse pyproject.toml: {}", e)))?;
        ...
        if let Some(tool) = toml_value.get("tool") {
            ...
            if let Some(fastest) = tool.get("fastest") {
                config.fastest = fastest.clone().try_into()
                    .map_err(|e| crate::error::Error::Config(format!("Failed to deserialize fastest config: {}", e)))?;
            }
            
            return Ok(config);
        }
        
        Err(crate::error::Error::Config("No pytest or fastest config found in pyproject.toml".to_string()))
    }
    ...
    pub fn load_from_pytest_ini() -> Result<Self> {
        ...
        if !path.exists() {
            return Err(crate::error::Error::Config("pytest.ini not found".to_string()));
        }
        
        let contents = fs::read_to_string(path)
            .map_err(|e| crate::error::Error::Config(format!("Failed to read pytest.ini: {}", e)))?;
    ...
    pub fn load_from_setup_cfg() -> Result<Self> {
        ...
        if !path.exists() {
            return Err(crate::error::Error::Config("setup.cfg not found".to_string()));
        }
        ...
        Err(crate::error::Error::Config("No pytest config found in setup.cfg".to_string()))
    }
    ...
    pub fn load_from_tox_ini() -> Result<Self> {
        ...
        Err(crate::error::Error::Config("No pytest config found in tox.ini".to_string()))
    }
