...
impl FixtureExecutor {
    ...
    fn build_fixture_execution_code(
        ...
        let module_name = test_path.file_stem()
...
try:
    ...
    sys.exit(1)
"#,
                fixture_name, fixture_name, fixture_name, fixture_name, fixture_name, fixture_name, fixture_name
            ));
        }
    ...
    let _fixture_kwargs = if test.fixture_deps.is_empty() {
        ...
        for fixture_name in &test.fixture_deps {
            if let Some(value) = fixture_values.get(fixture_name) {
                kwargs.push(format!("{}={}", fixture_name, serde_json::to_string(value).unwrap_or_else(|_| "None".to_string())));
            }
        }
        if kwargs.is_empty() {
            String::new()
