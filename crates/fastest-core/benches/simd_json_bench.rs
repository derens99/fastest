    ...
    for count in [100, 1000, 10000].iter() {
        let items: Vec<TestItem> = (0..*count)
            .map(|i| TestItem {
                ...
                class_name: if i % 10 == 0 {
                    Some(format!("TestClass{}", i / 10))
                } else {
                    None
                },
                is_xfail: false,
        ...
        group.bench_with_input(BenchmarkId::new("simd_parse", count), &json, |b, json| {
            b.iter(|| {
        ...
        group.bench_with_input(BenchmarkId::new("serde_parse", count), &json, |b, json| {
            b.iter(|| {
