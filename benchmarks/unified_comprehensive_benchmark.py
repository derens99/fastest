...
Usage:
    uv run python benchmarks/unified_comprehensive_benchmark.py [--quick] [--full] [--class-only]
...
class UnifiedComprehensiveBenchmark:
    ...
    def __init__(
        self, fastest_binary: str = "./target/release/fastest", mode: str = "full"
    ):
    ...
    def _create_test_files(
        self, test_dir: Path, num_tests: int, **kwargs
    ) -> List[Path]:
    ...
    def _create_simple_tests(
        self, test_dir: Path, num_tests: int, tests_per_file: int
    ) -> List[Path]:
...
'''
    ...
    def _run_fastest(
        self, test_path: Path, extra_args: List[str] = None
    ) -> Dict[str, Any]:
    ...
    def _calculate_speedup(
        self, fastest_time: float, pytest_time: Optional[float]
    ) -> Optional[float]:
    ...
    def benchmark_main_performance(self) -> List[UnifiedBenchmarkResult]:
        ...
        results = []
        test_sizes = (
            [10, 20, 50, 100, 500, 1000] if self.mode == "full" else [10, 50, 100]
        )
        ...
        for size in test_sizes:
            ...
            with self.create_temp_test_suite("main_perf", size) as (test_dir, _):
                ...
                result = UnifiedBenchmarkResult(
                    ...
                    strategy_used="auto",
                    error=(
                        None if fastest_result["success"] else fastest_result["stderr"]
                    ),
                )
                ...
                if speedup:
                    print(
                        f"      Fastest: {fastest_result['time']:.3f}s | Pytest: {pytest_result['time']:.3f}s | Speedup: {speedup:.2f}x"
                    )
                else:
                    print(
                        f"      Fastest: {fastest_result['time']:.3f}s | Pytest: Failed"
                    )
    ...
    def benchmark_scalability(self) -> List[UnifiedBenchmarkResult]:
        ...
        for size in test_sizes:
            ...
            with self.create_temp_test_suite("scalability", size) as (test_dir, _):
                ...
                fastest_result = self._run_fastest(
                    test_dir, ["--parser", "tree-sitter"]
                )
                ...
                speedup = self._calculate_speedup(
                    ...
                    (
                        pytest_result["time"]
                        if pytest_result and pytest_result["success"]
                        else None
                    ),
                )
                ...
                result = UnifiedBenchmarkResult(
                    ...
                    additional_metrics={
                        "tests_per_second": size / fastest_result["time"]
                    },
                    error=(
                        None if fastest_result["success"] else fastest_result["stderr"]
                    ),
                ...
                if speedup:
                    print(
                        f"      Fastest: {fastest_result['time']:.3f}s ({tps:.1f} tests/s) | Speedup: {speedup:.2f}x"
                    )
                else:
                    print(
                        f"      Fastest: {fastest_result['time']:.3f}s ({tps:.1f} tests/s)"
                    )
    ...
    def benchmark_parser_comparison(self) -> List[UnifiedBenchmarkResult]:
        ...
        with self.create_temp_test_suite("parser_comp", test_size) as (test_dir, _):
            ...
            for parser in parsers:
                ...
                fastest_result = self._run_fastest(
                    test_dir, ["--parser", parser, "--no-cache"]
                )
            ...
            if pytest_result["success"]:
                ...
                for i, parser in enumerate(parsers):
                    if results[i].fastest_time > 0:
                        results[i].speedup = (
                            pytest_result["time"] / results[i].fastest_time
                        )
    ...
    def benchmark_discovery_cache(self) -> List[UnifiedBenchmarkResult]:
        ...
        with self.create_temp_test_suite("cache_perf", test_size) as (test_dir, _):
            ...
            warm_benchmark = UnifiedBenchmarkResult(
                ...
                speedup=(
                    cold_result["time"] / warm_result["time"]
                    if warm_result["time"] > 0
                    else None
                ),
    ...
    def benchmark_class_based_execution(self) -> List[UnifiedBenchmarkResult]:
        ...
        for num_classes, tests_per_class in configurations:
            ...
            print(
                f"   🏛️  Testing {num_classes} classes with {tests_per_class} tests each..."
            )

            with self.create_temp_test_suite(
                "class_based",
                total_tests,
    ...
    def benchmark_complex_fixtures(self) -> List[UnifiedBenchmarkResult]:
        ...
        with self.create_temp_test_suite(
            "complex_fixtures", test_size, test_type="complex"
        ) as (test_dir, _):
    ...
    def benchmark_revolutionary_features(self) -> List[UnifiedBenchmarkResult]:
        ...
        for size in test_sizes:
            ...
            with self.create_temp_test_suite(
                "revolutionary", size, test_type="arithmetic"
            ) as (test_dir, _):
                ...
                strategy = (
                    "NativeJIT"
                    if size <= 20
                    else "BurstExecution" if size <= 100 else "UltraInProcess"
                )
                ...
                if speedup:
                    print(
                        f"      Fastest ({strategy}): {fastest_result['time']:.3f}s | Speedup: {speedup:.2f}x"
                    )
                else:
    ...
    def _print_comprehensive_summary(self, results: List[UnifiedBenchmarkResult]):
        ...
        print("\n📋 Detailed Results:")
        print(
            f"{'Benchmark':<30} {'Tests':<8} {'Fastest':<10} {'Pytest':<10} {'Speedup':<8} {'Strategy':<15}"
        )
        print("-" * 90)
        ...
        for category, cat_results in categories.items():
            cat_speedups = [
                r.speedup for r in cat_results if r.speedup and r.speedup > 0
            ]
            if cat_speedups:
        ...
        if valid_results:
            ...
            print(
                f"   Needs Work: {worst.benchmark_name} - {worst.speedup:.2f}x speedup"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Unified Comprehensive Benchmark Suite for Fastest"
    )
    parser.add_argument(
        "--mode",
        choices=["quick", "full", "class-only"],
        default="full",
        help="Benchmark mode: quick (essential tests), full (all tests), class-only (class tests only)",
    )
    parser.add_argument(
        "--binary", default="./target/release/fastest", help="Path to fastest binary"
    )
    parser.add_argument(
        "--output",
        default="benchmarks/unified_benchmark_results.json",
