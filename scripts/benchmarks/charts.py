#!/usr/bin/env python3
"""
Generate performance artifact charts for the Fastest project.

Charts visualize one benchmark artifact. Do not treat them as product claims
without the matching raw benchmark output and environment details.
"""
...
def load_benchmark_data():
    ...
    results_file = Path("target/benchmark-artifacts/quick/benchmark_results.json")
    if not results_file.exists():
        print(
            "❌ No benchmark results found. Run: uv run python scripts/benchmarks/official.py --quick --output-dir target/benchmark-artifacts/quick"
        )
        return None
...
def create_speedup_chart(data, output_dir):
    ...
    fastest_times = [
        c["fastest"]["total_time"] for c in comparisons if c.get("speedup_total")
    ]
    pytest_times = [
        c["pytest"]["total_time"] for c in comparisons if c.get("speedup_total")
    ]

    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 6))

    # Chart 1: Ratio factor
    ...
    ax1.set_ylabel("Ratio vs pytest", fontsize=14, fontweight="bold")
    ax1.set_title("Fastest vs pytest - Benchmark Ratio", fontsize=16, fontweight="bold")
    ...
    ax1.axhline(y=1, color="red", linestyle="--", alpha=0.7, label="Parity")
    ax1.legend()
...
def create_summary_chart(speedups, test_counts, data, output_dir):
    ...
    ax1.set_title("Performance Ratio vs pytest", fontweight="bold")
    ax1.set_xticks(range(len(test_counts)))
    ax1.set_xticklabels([f"{count}" for count in test_counts])
    ax1.set_ylabel("Ratio vs pytest")
    ax1.grid(True, alpha=0.3)
    ...
    if len(speedups) >= 3:
        cat_speedups = [
            ...
            (
                np.mean(speedups[3:5])
                if len(speedups) > 4
                else speedups[3] if len(speedups) > 3 else speedups[-1]
            ),  # Middle for medium
            (
                np.mean(speedups[5:]) if len(speedups) > 5 else speedups[-1]
            ),  # Last for large
        ]
    else:
    ...
    ax2.set_title("Performance Ratio by Category", fontweight="bold")
...
def main():
    ...
    # Generate charts
    print("📊 Creating benchmark ratio chart...")
    ...
    print("=" * 50)
    print(f"📊 Average Ratio vs pytest: {avg_speedup:.1f}x")
    print(f"🚀 Maximum Ratio vs pytest: {max_speedup:.1f}x")
